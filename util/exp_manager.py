...
class ExperimentManager:
    ...
    def update_csv(
        ...
        sensor_aurocs: dict = None,  # 新增参数，用于传入 sensor_aurocs_
    ):
        """
        ...
        # 如果提供了 sensor_aurocs，将其加入 new_rows
        if sensor_aurocs is not None:
        # 将 sensor_aurocs 的键值对作为列添加到 new_rows
            #sensor_cols = {f"sensor_{k}_auroc": v for k, v in sensor_aurocs.items()}
            sensor_cols = {"auroc"}
            sensor_df = pd.DataFrame([sensor_cols])
            new_rows = pd.concat((new_rows, sensor_df), axis=1)
